...
def _render_sidebar() -> dict:
    ...
    if mode == "regional":
        sat_default = "modis"
        st.sidebar.info("Regional mode uses MODIS (1 km) automatically.")
...
def _render_date_params(mode: str) -> dict:
    ...
    if mode == "single":
        ...
        sel_reg    = st.sidebar.selectbox(
            "Region:", reg_labels, index=1, key="serves_region"   # default: Europe
        )
        ...
        st.sidebar.caption("⚠️ Uses MODIS (1 km). Continental analysis takes several minutes.")
...
def _render_geometry_selection() -> bool:
    ...
    if st.session_state.get("serves_geometry") is not None:
        ...
        try:
            area_km2 = (
                st.session_state.serves_geometry.area().divide(1e6).getInfo()
            )
        ...
        try:
            if mode == "single":
                ...
                if params.get("multi_year"):
                    ...
                    count  = result["image_count"].getInfo()
                    if not count:
                        st.error(f"❌ No images found for {period}.")
                        return

                st.session_state["serves_results"] = {
                    "type": "period", "image": result["soil_moisture"],
                    "ndvi": result.get("ndvi"), "period_label": period,
                    "image_count": count, "params": params,
                }
                ...
                count = result["image_count"].getInfo()
                if not count:
                    st.error("❌ No images found for this date range.")
                    return
...
def _render_regional_mode(params: dict):
    ...
    st.info(
        "This mode uses **MODIS (1 km)** to compute multi-year soil moisture climatology "
        "over predefined continental regions. No custom AOI is needed. "
        "⚠️ Processing may take several minutes for large regions."
    )

    region_label = PREDEFINED_REGION_LABELS.get(params.get("region", "europe"), "Europe")
    period_label = (f"{get_month_name(params['month'])} "
                    f"{params['start_year']}–{params['end_year']}")
    ...
    if run_clicked:
        ...
        with st.spinner(f"🌍 Computing regional climatology for {region_label}…"):
            try:
                ...
                vis_key = "soil_moisture" if stat_key != "std_dev" else "et_fraction"
...
def _render_regional_results():
    ...
    col1.metric("Region",     PREDEFINED_REGION_LABELS.get(cached["region"], cached["region"]))
    ...
    with st.spinner("Computing statistics…"):
        try:
            stats = get_image_statistics(
                image, study_area, scale=params.get("scale", 500), band="soil_moisture"
            )
...
def _render_map_with_layer(image, vis_params: dict, region, layer_name: str,
    ...
    """Build a Folium map with a GEE tile layer + embedded colorbar and display it."""
    ...
    if region_name and region_name in PREDEFINED_REGION_COORDS:
        ...
        try:
            ...
            zoom = 9 if area_km2 < 1000 else (7 if area_km2 < 50000 else 4)
        except Exception:
            center_lat, center_lon, zoom = 20, 0, 3
    ...
    if ndvi_image is not None:
        try:
            ndvi_map_id = ndvi_image.getMapId(VIS_PARAMS["ndvi"])
            folium.TileLayer(
    ...
    # ── Colorbar embedded inside the map ────────────────────────────────
    LinearColormap(
...
def _build_period_label(params: dict) -> str:
    ...
    if mode == "time_series":
        return f"{params.get('start_date')} → {params.get('end_date')}"
    if mode == "regional":
        r = PREDEFINED_REGION_LABELS.get(params.get("region", "europe"), "Europe")
        return f"{r} · {get_month_name(params.get('month', 1))} {params.get('start_year')}–{params.get('end_year')}"
    return mode
